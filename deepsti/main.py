...
from lib.utils import chooseLoss, chooseModel, chooseOptimizer, loadData, prepareDataset
...
vis_output_path = args.output_path  # reconstruction results
...
def main(args):
    ...
    if args.mode == "train":
        ...
        ## load dataset
        print("Load dataset...")
        train_loader, val_loader, test_loader = prepareDataset(
            args, device, npy_dir, data_normalization, args.n_w
        )
        ...
        for epoch in range(start_epoch, args.num_epoch):
            ...
            mse_index, vec_index, wpsnr_index = validate(
                device, model, val_loader, epoch, loss_fn, tb_writer, split="val"
            )
            _, _, _ = validate(
                device, model, test_loader, epoch, loss_fn, tb_writer, split="test"
            )
            ...
            state = {
                ...
                "best_mse_index": best_mse_index,
                "best_vec_index": best_vec_index,
            }
        ...
        data_loader = loadData(
            args, device, npy_dir, prediction_data, data_normalization
        )
        ...
        if data_normalization and len(args.ext_data) > 0:
            data_loader.dataset.set_mean_std(
                model.gt_mean.squeeze().numpy(), model.gt_std.squeeze().numpy()
            )
        ...
        if not args.resume_file == None:
            model.load_state_dict(
                torch.load(args.resume_file, map_location=device)["model_state"]
            )
...
def train(
    args, device, model, train_loader, epoch, loss_fn, optimizer, tb_writer, total_tb_it
):
    ...
    for batch_count, (
        ...
        _,
        sub_name,
    ) in enumerate(tqdm(train_loader, ncols=60)):
...
def validate(device, model, val_loader, epoch, loss_fn, tb_writer, split):
    ...
    with torch.no_grad():
        ...
        for batch_count, (
            ...
            _,
            sub_name,
        ) in enumerate(tqdm(val_loader, ncols=50)):
            ...
            if data_normalization:
                og_output = (
                    torch.squeeze(output_data, 0).permute(1, 2, 3, 0).cpu().numpy()
                    * val_loader.dataset.gt_std
                )
                og_output = og_output + val_loader.dataset.gt_mean
                og_output = og_output * mask[:, :, :, np.newaxis]

                og_gt = (
                    torch.squeeze(gt_data, 0).permute(1, 2, 3, 0).cpu().numpy()
                    * val_loader.dataset.gt_std
                )
        ...
        print(
            "##",
            split,
            "loss: %.8f Mse: %.8f PSNR: %.8f SSIM: %.8f VEC: %.8f WPSNR: %.8f"
            % (
                avg_tb_loss,
                avg_mse_loss,
                avg_psnr_perf,
    ...
    with torch.no_grad():
        for batch_count, (
            ...
            if not args.no_save:
                ...
                if prediction_set == "ext":
                    save_name = (
                        vis_output_path + "/" + model_name + "/" + sub_name[0] + "_pred"
                    )
                else:
                    save_name = (
                        vis_output_path
                        ...
                        + "/"
                        + sub_name[0]
                        + "_snr{}".format(data_loader.dataset.snr)
                        + "_pred"
                    )
